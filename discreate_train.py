...
from transformers import AutoTokenizer, get_linear_schedule_with_warmup, get_cosine_schedule_with_warmup, \
    get_constant_schedule_with_warmup, GPTNeoXForCausalLM
...
from src.utils.constants import DEFAULT_PAD_TOKEN, DEFAULT_EOS_TOKEN, DEFAULT_BOS_TOKEN, DEFAULT_UNK_TOKEN, \
    DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, DIS_IMG_TOKEN
...
def main():
    ...
    if args.model_name.lower() == "pythia":
    ...
    image_token_list = [DIS_IMG_TOKEN.format(int(i)) for i in range(0,8192)]
    num_new_tokens = tokenizer.add_tokens(image_token_list, special_tokens=True)
    model.resize_token_embeddings(len(tokenizer))
    ...
    # test
    # a1 = tokenizer.convert_tokens_to_ids(DEFAULT_IMAGE_PATCH_TOKEN)
    # b2 = tokenizer.convert_tokens_to_ids(DEFAULT_IM_START_TOKEN)
    # c3 = tokenizer.convert_tokens_to_ids(DEFAULT_IM_END_TOKEN)
    # a = tokenizer.convert_tokens_to_ids('<im_00000>')
    # b = tokenizer.convert_tokens_to_ids('<im_08191>')
    # c = tokenizer.convert_tokens_to_ids('<im_08192>')
