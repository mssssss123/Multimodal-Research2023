...
from src.utils.constants import bert_checkpoint_path
...
class Blip2Base(nn.Module):
    ...
    def init_tokenizer(cls, truncation_side="right"):
        # TODO:这里需要手动修改路径
        tokenizer = BertTokenizer.from_pretrained(bert_checkpoint_path, truncation_side=truncation_side)
    ...
    def init_Qformer(cls, num_query_token, vision_width, cross_attention_freq=2):
        # TODO:这里需要手动修改路径
        encoder_config = BertConfig.from_pretrained(bert_checkpoint_path)
        encoder_config.encoder_width = vision_width
        # insert cross-attention layer every other block
        encoder_config.add_cross_attention = True
        encoder_config.cross_attention_freq = cross_attention_freq
        encoder_config.query_length = num_query_token
        Qformer = BertLMHeadModel.from_pretrained(bert_checkpoint_path, config=encoder_config)
